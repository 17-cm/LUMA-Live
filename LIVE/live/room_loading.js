...
(function initRoomLoadingModule() {
  ...
  function showHostLeftRoomStage(session) {
    ...
    // 立即彻底停止正在进行的推流定时器与语音播放
    try {
      if (typeof window.liveDurationInterval !== 'undefined') clearInterval(window.liveDurationInterval);
      if (typeof window.danmakuDripTimer !== 'undefined') clearInterval(window.danmakuDripTimer);
      if (typeof window.hostSpeechDripTimer !== 'undefined') clearInterval(window.hostSpeechDripTimer);
      if (typeof window.viewerCountInterval !== 'undefined') clearInterval(window.viewerCountInterval);
      if (window.api && window.api.voice && window.api.voice.stopPlayback) {
        window.api.voice.stopPlayback({ channel: "voice" });
      }
    } catch (e) {}
    ...
    if (followBtn && s.characterId) {
      const isFollowed = (window.followedHosts || []).includes(s.characterId);
      followBtn.innerHTML = isFollowed ? '已关注' : '<span>+</span> 关注';
      if (isFollowed) followBtn.classList.add('followed');
      else followBtn.classList.remove('followed');
    }
