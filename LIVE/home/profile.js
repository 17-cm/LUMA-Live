...
async function syncUserProfile() {
  ...
  const myFans = window.LumaFansManager ? window.LumaFansManager.getFans('user') : (userProfileData.fans || 0);
  ...
  if (statLikeEl) statLikeEl.textContent = (userProfileData.likes || 0).toLocaleString();
    ...
    return `
      <div class="luxe-card p-3 flex items-center justify-between ${m ? m.bg : 'bg-white'} border transition-all duration-200 hover:shadow-md">
        ...
        <div class="text-right flex-shrink-0">
          ...
          <span class="text-[8px] text-slate-400 block mt-0.5" ${item.lastTimestamp ? `data-dynamic-time data-ts="${item.lastTimestamp}"` : ''}>${item.lastTimestamp ? (window.formatDynamicTime ? window.formatDynamicTime(item.lastTimestamp) : '活跃') : '活跃'}</span>
        </div>
      </div>
    `;
  }).join('');
