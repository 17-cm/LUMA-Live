...
function getOrGenerateStreamerProfile(characterId, characterObj) {
  ...
  // 真实直播历史场次（优先从离线推演/真实数据库 streamer_history 中聚合）
  let dbHistory = [];
  try {
    if (window.db) {
      const allHist = (window.db.get ? window.db.get("streamer_history") : null) || [];
      // 如果是 Promise
      if (allHist && typeof allHist.then === 'function') {
        // sync 回退
      } else if (Array.isArray(allHist)) {
        dbHistory = allHist.filter(h => h.characterId === characterId);
      }
    }
  } catch (e) {}
  ...
  // 若数据库中有真实/推演沉淀的场次，直接使用真实时间戳转换
  if (dbHistory.length > 0) {
    dbHistory.slice(0, 15).forEach((h, idx) => {
      const showNum = (totalShows || dbHistory.length) - idx;
      const hDur = Math.floor(h.durationMin / 60);
      const mDur = h.durationMin % 60;
      showsHistory.push({
        showNumber: showNum,
        title: h.title || `第 ${showNum} 场 · ${titlesPool[(hash + idx) % titlesPool.length]}`,
        duration: hDur > 0 ? `${hDur}小时${mDur}分` : `${mDur}分钟`,
        heat: (h.peakViewers ? h.peakViewers * 40 : 35000).toLocaleString(),
        newFans: `+${h.fansGained || 320} 粉丝`,
    ...
    for (let i = 0; i < Math.min(8, totalShows); i++) {
      ...
      const m = durMins % 60;
      // 模拟过去 i 天的时间戳
      const pastTs = Date.now() - (i === 0 ? 30 * 60 * 1000 : i * 24 * 60 * 60 * 1000);
      showsHistory.push({
        showNumber: showNum,
  ...
  box.innerHTML = list.length === 0 ? '<p class="text-[11px] text-slate-400 py-3 text-center">暂无留言，快来给主播抢个沙发吧~</p>' : list.map(m => `
    <div class="bg-white p-3 rounded-2xl border border-slate-100 shadow-xs space-y-1.5">
      <div class="flex justify-between text-[10px]">
        ...
        <span class="text-slate-400" ${m.createdAt ? `data-dynamic-time data-ts="${m.createdAt}"` : ''}>${m.createdAt ? (window.formatDynamicTime ? window.formatDynamicTime(m.createdAt) : '刚刚') : (m.time || '刚刚')}</span>
      </div>
