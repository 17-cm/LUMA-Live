...
async function handleGenerateNewTrend() {
  ...
  const newPostCreatedAt = Date.now();
  const newPost = {
    id: `post_${Date.now()}`,
    author: {
      name: `${host.name}超话前线`,
    ...
    commentTree: (parsed.comments || []).map((c, i) => {
      const commentTs = window.TimeKeeper ? window.TimeKeeper.generateCausalChildTimestamp(newPostCreatedAt, i, parsed.comments.length) : Date.now();
      return {
        id: `c_${Date.now()}_${i}`,
        user: c.user || '吃瓜群众',
