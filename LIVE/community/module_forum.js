      ...
      <div class="flex items-start justify-between">
        <div class="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p class="text-[9px] text-slate-400 mt-0.5">${(function () { const tail = post.time && post.time.indexOf('来自') >= 0 ? '来自 ' + post.time.split('来自').pop().trim() : ''; return post.createdAt ? `<span data-dynamic-time data-ts="${post.createdAt}">${window.formatDynamicTime ? window.formatDynamicTime(post.createdAt) : '刚刚'}</span>${tail ? ' · ' + tail : ''}` : (post.time || '刚刚'); })()}</p>
          </div>
        </div>
          ...
          ${(post.comments || []).map(comm => `
            <div class="p-2.5 rounded-xl bg-slate-50 text-xs space-y-1">
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                  <span class="text-[8px] text-slate-400" ${comm.createdAt ? `data-dynamic-time data-ts="${comm.createdAt}"` : ''}>${(comm.createdAt && window.formatDynamicTime) ? window.formatDynamicTime(comm.createdAt) : (comm.time || '刚刚')}</span>
